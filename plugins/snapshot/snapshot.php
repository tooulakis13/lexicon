...
if (!class_exists('WPMUDEVSnapshot')) {
	class WPMUDEVSnapshot {
		...
		function __construct() {
			...
			$this->DEBUG									= false;
			$this->_settings['SNAPSHOT_VERSION'] 			= '2.4.0.7';
					...
					foreach($tables_set as $table_name) {
						if ($section != "global") {
							...
							if ($table_name == $wpdb->base_prefix."users") {
								$sql_str = "SELECT ID FROM ". $wpdb->base_prefix ."users WHERE ID IN (". join(',', $this->_session->data['global_user_ids']).")";
								$sql_table = "SELECT * FROM ". $wpdb->base_prefix ."users WHERE ID IN (". join(',', $this->_session->data['global_user_ids']).")";

								//echo "sql_str=[". $sql_str ."]<br />";
							} else if ($table_name == $wpdb->base_prefix."usermeta") {

								$sql_str = "SELECT umeta_id FROM ". $wpdb->base_prefix ."usermeta WHERE user_id IN (". join(',', $this->_session->data['global_user_ids']).")";
								$sql_table = "SELECT * FROM ". $wpdb->base_prefix ."usermeta WHERE user_id IN (". join(',', $this->_session->data['global_user_ids']).")";
								//echo "sql_str=[". $sql_str ."]<br />";
							}
							$row_ids = $wpdb->get_col($sql_str);
							if ($row_ids) {
								$_set['table_name'] 		= $table_name;
								$_set['rows_total'] 		= count($row_ids);
								$_set['segment_idx'] 		= 1;
								$_set['segment_total'] 		= 1;
								$_set['rows_start'] 		= 0;
								$_set['rows_end'] 			= 0;
